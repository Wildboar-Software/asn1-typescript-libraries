/* eslint-disable */
import { CURVES } from "../ANSI-X9-62/CURVES.oca";
import { prime256v1 } from "../ANSI-X9-62/prime256v1.va";
export { CURVES } from "../ANSI-X9-62/CURVES.oca";
export { prime256v1 } from "../ANSI-X9-62/prime256v1.va";

/* START_OF_SYMBOL_DEFINITION CurveNames_Union26_Intersection0_Element */
/**
 * @summary CurveNames_Union26_Intersection0_Element
 * @description
 *
 * ### ASN.1 Definition:
 *
 * ```asn1
 * CurveNames-Union26-Intersection0-Element ::= {} -- REMOVED_FROM_UNNESTING --
 * ```
 *
 * @constant
 * @type {CURVES}
 * @implements {CURVES}
 */
export const CurveNames_Union26_Intersection0_Element: CURVES = {
    class: "CURVES",
    decoderFor: {},
    encoderFor: {},
    "&id": prime256v1 /* OBJECT_FIELD_SETTING */ /* UNIQUE_OBJECT_FIELD_SETTING */,
};
/* END_OF_SYMBOL_DEFINITION CurveNames_Union26_Intersection0_Element */

/* eslint-enable */
