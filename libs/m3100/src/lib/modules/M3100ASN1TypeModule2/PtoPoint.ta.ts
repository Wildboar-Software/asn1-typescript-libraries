    ...
    _cached_decoder_for_PtoPoint = function (el: _Element): PtoPoint {
      ...
      if (sequence.length < 2) {
        throw new _ConstructionError(
          'PtoPoint contained only ' + sequence.length.toString() + ' elements.'
        );
      }
    ...
    _cached_encoder_for_PtoPoint = function (
      ...
      return $._encodeSequence(
        ([] as (_Element | undefined)[])
          .concat([
            /* REQUIRED   */ _encode_ConnectivityEndPoint(value.aEnd, $.BER),
            /* REQUIRED   */ _encode_ConnectivityEndPoint(value.zEnd, $.BER),
          ])
