...
for pkg in "${packages[@]}"; do
  git checkout "onboard_${pkg}"
  npx organize-imports-cli packages/${pkg}/src/lib/modules
  git add packages/${pkg}/src/lib/modules
  git commit -m "${pkg}: remove unused imports"
  git push
  git checkout master
