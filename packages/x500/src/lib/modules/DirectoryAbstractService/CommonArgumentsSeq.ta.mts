 ...
 * @description
 *
 * SEQUENCE of the same components as `CommonArguments` (for SEQUENCE
 * COMPONENT OF). `serviceControls` / `securityParameters` absence ≡
 * empty. Modern DUAs/DSAs shall omit `aliasedRDNs`. `criticalExtensions`
 * bit 0 is unused. `familyGrouping` DEFAULT `entryOnly`.
 * `operationContexts` `allContexts` overrides DSA defaults.
 *
 * ### ASN.1 Definition:
 *
 * ```asn1
 ...
 */
export function _decode_CommonArgumentsSeq(el: _Element): CommonArgumentsSeq {
...
export function _encode_CommonArgumentsSeq(
    ...
    if (!_cached_encoder_for_CommonArgumentsSeq) {
        _cached_encoder_for_CommonArgumentsSeq = function (
            value: CommonArgumentsSeq        ): _Element {
