...
function getAttributeSize (attr: Attribute): number {
    let size: number =
        2 // TAG and length for outermost SEQUENCE
        + 2 // TAG and length for type
        + attr.type_.byteLength()
        + 2 // TAG and length for values
        + (attr.valuesWithContext?.length ? 2 : 0)
        ;
    ...
    for (const vwc of attr.valuesWithContext ?? []) {
        let sizeOfVWC: number = (
            ...
            + BYTES_FOR_TAG // For the whole SEQUENCE
            + BYTES_FOR_TAG // for the SET of contexts
        );
        for (const context of vwc.contextList) {
            let sizeOfContext = (
                BYTES_FOR_TAG
                + context.contextType.byteLength()
...
export default getAttributeSize;
