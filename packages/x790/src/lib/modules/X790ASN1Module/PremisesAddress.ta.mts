...
class PremisesAddress {
    constructor (
        ...
        for (const [name, v] of [
            ...
            if (v.length > 64) {
                throw new ASN1SizeError(`PremisesAddress.${name} violates SIZE constraint`);
            }
        }
    }
     ...
     */
    public static _from_object (_o: { [_K in keyof (PremisesAddress)]: (PremisesAddress)[_K] }): PremisesAddress {
        return new PremisesAddress(_o.civicAddress, _o.city, _o.state, _o.zip, _o._unrecognizedExtensionsList);
    }
 ...
 */
export
const _root_component_type_list_1_spec_for_PremisesAddress: $.ComponentSpec[] = [
    new $.ComponentSpec("civicAddress", false, $.hasTag(_TagClass.universal, 25)),
    new $.ComponentSpec("city", false, $.hasTag(_TagClass.universal, 25)),
    new $.ComponentSpec("state", false, $.hasTag(_TagClass.universal, 25)),
    new $.ComponentSpec("zip", false, $.hasTag(_TagClass.universal, 25))
];
 ...
 */
export
const _root_component_type_list_2_spec_for_PremisesAddress: $.ComponentSpec[] = [
 ...
 */
export
const _extension_additions_list_spec_for_PremisesAddress: $.ComponentSpec[] = [
 ...
 */
export
function _decode_PremisesAddress (el: _Element): PremisesAddress {
    if (!_cached_decoder_for_PremisesAddress) { _cached_decoder_for_PremisesAddress = function (el: _Element): PremisesAddress {
    const sequence: _Element[] = el.sequence;
    if (sequence.length < 4) {
        throw new _ConstructionError("PremisesAddress contained only " + sequence.length.toString() + " elements.");
    }
 ...
 */
export
function _encode_PremisesAddress (value: PremisesAddress, elGetter: $.ASN1Encoder<any>): _Element {
    if (!_cached_encoder_for_PremisesAddress) { _cached_encoder_for_PremisesAddress = function (value: PremisesAddress, elGetter: $.ASN1Encoder<PremisesAddress>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ $._encodeGraphicString(value.civicAddress, $.BER),
            /* REQUIRED   */ $._encodeGraphicString(value.city, $.BER),
            /* REQUIRED   */ $._encodeGraphicString(value.state, $.BER),
            /* REQUIRED   */ $._encodeGraphicString(value.zip, $.BER)
        ],
