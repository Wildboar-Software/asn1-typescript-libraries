 ...
 */
export const _decode_MME_EventList = (el: _Element): MME_EventList => {
    const value = $._decodeBitString(el);
    if (value.length < 6 || value.length > 8) {
        throw new ASN1SizeError("MME_EventList violates SIZE constraint");
    }
    return value;
};
 ...
 */
export const _encode_MME_EventList = $._encodeBitString;
