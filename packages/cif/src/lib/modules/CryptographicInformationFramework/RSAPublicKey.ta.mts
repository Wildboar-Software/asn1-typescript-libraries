...
class RSAPublicKey {
     ...
     */
    public static _from_object (_o: { [_K in keyof (RSAPublicKey)]: (RSAPublicKey)[_K] }): RSAPublicKey {
 ...
 */
export
const _root_component_type_list_1_spec_for_RSAPublicKey: $.ComponentSpec[] = [
 ...
 */
export
const _root_component_type_list_2_spec_for_RSAPublicKey: $.ComponentSpec[] = [
 ...
 */
export
const _extension_additions_list_spec_for_RSAPublicKey: $.ComponentSpec[] = [
 ...
 */
export
function _decode_RSAPublicKey (el: _Element): RSAPublicKey {
    if (!_cached_decoder_for_RSAPublicKey) { _cached_decoder_for_RSAPublicKey = function (el: _Element): RSAPublicKey {
    const sequence: _Element[] = el.sequence;
    if (sequence.length < 2) {
        throw new _ConstructionError("RSAPublicKey contained only " + sequence.length.toString() + " elements.");
    }
 ...
 */
export
function _encode_RSAPublicKey (value: RSAPublicKey, elGetter: $.ASN1Encoder<any>): _Element {
    if (!_cached_encoder_for_RSAPublicKey) { _cached_encoder_for_RSAPublicKey = function (value: RSAPublicKey, elGetter: $.ASN1Encoder<RSAPublicKey>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ $._encodeInteger(value.modulus, $.BER),
            /* REQUIRED   */ $._encodeInteger(value.publicExponent, $.BER)
        ],
