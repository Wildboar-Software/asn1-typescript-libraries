...
class SubjectAltPublicKeyInfo {
     ...
     */
    public static _from_object (_o: { [_K in keyof (SubjectAltPublicKeyInfo)]: (SubjectAltPublicKeyInfo)[_K] }): SubjectAltPublicKeyInfo {
        return new SubjectAltPublicKeyInfo(_o.algorithm, _o.subjectAltPublicKey);
    }
 ...
 */
export
const _root_component_type_list_1_spec_for_SubjectAltPublicKeyInfo: $.ComponentSpec[] = [
    new $.ComponentSpec("algorithm", false, $.hasTag(_TagClass.universal, 16)),
    new $.ComponentSpec("subjectAltPublicKey", false, $.hasTag(_TagClass.universal, 3))
];
 ...
 */
export
const _root_component_type_list_2_spec_for_SubjectAltPublicKeyInfo: $.ComponentSpec[] = [
 ...
 */
export
const _extension_additions_list_spec_for_SubjectAltPublicKeyInfo: $.ComponentSpec[] = [
 ...
 */
export
function _decode_SubjectAltPublicKeyInfo (el: _Element): SubjectAltPublicKeyInfo {
    if (!_cached_decoder_for_SubjectAltPublicKeyInfo) { _cached_decoder_for_SubjectAltPublicKeyInfo = function (el: _Element): SubjectAltPublicKeyInfo {
    const sequence: _Element[] = el.sequence;
    if (sequence.length < 2) {
        throw new _ConstructionError("SubjectAltPublicKeyInfo contained only " + sequence.length.toString() + " elements.");
    }
 ...
 */
export
function _encode_SubjectAltPublicKeyInfo (value: SubjectAltPublicKeyInfo, elGetter: $.ASN1Encoder<any>): _Element {
    if (!_cached_encoder_for_SubjectAltPublicKeyInfo) { _cached_encoder_for_SubjectAltPublicKeyInfo = function (value: SubjectAltPublicKeyInfo, elGetter: $.ASN1Encoder<SubjectAltPublicKeyInfo>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ _encode_AlgorithmIdentifier(value.algorithm, $.BER),
            /* REQUIRED   */ $._encodeBitString(value.subjectAltPublicKey, $.BER)
        ],
