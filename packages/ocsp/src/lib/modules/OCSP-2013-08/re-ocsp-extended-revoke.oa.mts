/* eslint-disable */
import { NULL } from "@wildboar/asn1";
import * as $ from "@wildboar/asn1/functional";
import type { EXTENSION } from "@wildboar/pki-stub";
import { id_pkix_ocsp_extended_revoke } from "../OCSP-2013-08/id-pkix-ocsp-extended-revoke.va.mjs";


/**
 * @summary re_ocsp_extended_revoke
 * @description
 *
 * Response extension (NULL value) indicating support for the
 * extended definition of `revoked` to cover non-issued certificates,
 * identified by `id-pkix-ocsp-extended-revoke`
 * ([RFC 6960 §4.4.8](https://datatracker.ietf.org/doc/html/rfc6960#section-4.4.8),
 * [§2.2](https://datatracker.ietf.org/doc/html/rfc6960#section-2.2)).
 * MUST be in `responseExtensions`; MUST NOT be marked critical; MUST
 * be included when returning `revoked` for a non-issued certificate.
 *
 * ### ASN.1 Definition:
 *
 * ```asn1
 * re-ocsp-extended-revoke EXTENSION ::= { SYNTAX NULL IDENTIFIED BY
 *                                     id-pkix-ocsp-extended-revoke }
 * ```
 *
 * @constant
 * @type {EXTENSION<NULL>}
 * @implements {EXTENSION<NULL>}
 */
export const re_ocsp_extended_revoke: EXTENSION<NULL> = {
    class: "EXTENSION",
    decoderFor: {
        "&ExtnType": $._decodeNull,
    },
    encoderFor: {
        "&ExtnType": $._encodeNull,
    },
    "&id": id_pkix_ocsp_extended_revoke /* OBJECT_FIELD_SETTING */ /* UNIQUE_OBJECT_FIELD_SETTING */,
    "&ExtnType": 0 as never /* OBJECT_FIELD_SETTING OBJECT_TYPE_FIELD_SETTING */,
};

/* eslint-enable */
