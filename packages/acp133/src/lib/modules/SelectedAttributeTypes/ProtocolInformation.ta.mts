...
class ProtocolInformation {
     ...
     */
    public static _from_object (_o: { [_K in keyof (ProtocolInformation)]: (ProtocolInformation)[_K] }): ProtocolInformation {
 ...
 */
export
const _root_component_type_list_1_spec_for_ProtocolInformation: $.ComponentSpec[] = [
    new $.ComponentSpec("nAddress", false, $.hasTag(_TagClass.universal, 4)),
    new $.ComponentSpec("profiles", false, $.hasTag(_TagClass.universal, 17))
];
 ...
 */
export
const _root_component_type_list_2_spec_for_ProtocolInformation: $.ComponentSpec[] = [
 ...
 */
export
const _extension_additions_list_spec_for_ProtocolInformation: $.ComponentSpec[] = [
...
function _decode_ProtocolInformation (el: _Element): ProtocolInformation {
    if (!_cached_decoder_for_ProtocolInformation) { _cached_decoder_for_ProtocolInformation = function (el: _Element): ProtocolInformation {
    const sequence: _Element[] = el.sequence;
    if (sequence.length < 2) {
        throw new _ConstructionError("ProtocolInformation contained only " + sequence.length.toString() + " elements.");
    }
    ...
    profiles = $._decodeSetOf<OBJECT_IDENTIFIER>(() => $._decodeObjectIdentifier)(sequence[1]);
...
function _encode_ProtocolInformation (value: ProtocolInformation, elGetter: $.ASN1Encoder<any>): _Element {
    if (!_cached_encoder_for_ProtocolInformation) { _cached_encoder_for_ProtocolInformation = function (value: ProtocolInformation, elGetter: $.ASN1Encoder<ProtocolInformation>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ $._encodeOctetString(value.nAddress, $.BER),
            /* REQUIRED   */ $._encodeSetOf<OBJECT_IDENTIFIER>(() => $._encodeObjectIdentifier, $.BER)(value.profiles, $.BER)
        ],
