...
} from "../PkiPmiExternalDataTypes/BuiltInDomainDefinedAttribute.ta.mjs";
 ...
 * SEQUENCE is the encoded order; MDs may define extra equivalence rules for relative
 * order (X.402 §18.4 NOTE). SIZE (1..4): omit the OR-address component rather than send
 * empty. Type and value are both PrintableString; teletex/universal twins are extension
 * attributes. Both type and value must use the same string repertoire (X.402 §18.2 a).
 *
 * ### ASN.1 Definition:
 *
 * ```asn1
 ...
 */
export function _encode_BuiltInDomainDefinedAttributes(value: BuiltInDomainDefinedAttributes, elGetter: $.ASN1Encoder<BuiltInDomainDefinedAttributes>): _Element {
