...
class PA_DATA {
     ...
     * @returns {PA_DATA}
     */
    public static _from_object (_o: { [_K in keyof (PA_DATA)]: (PA_DATA)[_K] }): PA_DATA {
 ...
 */
export
const _root_component_type_list_1_spec_for_PA_DATA: $.ComponentSpec[] = [
    new $.ComponentSpec("padata-type", false, $.hasTag(_TagClass.context, 1)),
    new $.ComponentSpec("padata-value", false, $.hasTag(_TagClass.context, 2))
];
 ...
 */
export
const _root_component_type_list_2_spec_for_PA_DATA: $.ComponentSpec[] = [
 ...
 */
export
const _extension_additions_list_spec_for_PA_DATA: $.ComponentSpec[] = [
 ...
 */
export
function _decode_PA_DATA (el: _Element): PA_DATA {
    if (!_cached_decoder_for_PA_DATA) { _cached_decoder_for_PA_DATA = function (el: _Element): PA_DATA {
    const sequence: _Element[] = el.sequence;
    if (sequence.length < 2) {
        throw new _ConstructionError("PA-DATA contained only " + sequence.length.toString() + " elements.");
    }
    ...
    padata_value = $._decode_explicit<OCTET_STRING>(() => $._decodeOctetString)(sequence[1]);
    return new PA_DATA(
        padata_type,
        padata_value,
 ...
 */
export
function _encode_PA_DATA (value: PA_DATA, elGetter: $.ASN1Encoder<any>): _Element {
    if (!_cached_encoder_for_PA_DATA) { _cached_encoder_for_PA_DATA = function (value: PA_DATA, elGetter: $.ASN1Encoder<PA_DATA>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ $._encode_explicit(_TagClass.context, 1, () => _encode_Int32, $.BER)(value.padata_type, $.BER),
            /* REQUIRED   */ $._encode_explicit(_TagClass.context, 2, () => $._encodeOctetString, $.BER)(value.padata_value, $.BER)
        ],
