 ...
 */
export
const _root_component_type_list_1_spec_for_Ticket: $.ComponentSpec[] = [
    new $.ComponentSpec("tkt-vno", false, $.hasTag(_TagClass.context, 0)),
    new $.ComponentSpec("realm", false, $.hasTag(_TagClass.context, 1)),
    new $.ComponentSpec("sname", false, $.hasTag(_TagClass.context, 2)),
 ...
 */
export
const _root_component_type_list_2_spec_for_Ticket: $.ComponentSpec[] = [
 ...
 */
export
const _extension_additions_list_spec_for_Ticket: $.ComponentSpec[] = [
 ...
 */
export
function _decode_Ticket (el: _Element): Ticket {
    if (!_cached_decoder_for_Ticket) { _cached_decoder_for_Ticket = $._decode_implicit<Ticket>(() => function (el: _Element): Ticket {
    const sequence: _Element[] = el.sequence;
    if (sequence.length < 4) {
        throw new _ConstructionError("Ticket contained only " + sequence.length.toString() + " elements.");
    }
    ...
    tkt_vno = $._decode_explicit<INTEGER>(() => $._decodeInteger)(sequence[0]);
    realm = $._decode_explicit<Realm>(() => _decode_Realm)(sequence[1]);
    sname = $._decode_explicit<PrincipalName>(() => _decode_PrincipalName)(sequence[2]);
 ...
 */
export
function _encode_Ticket (value: Ticket, elGetter: $.ASN1Encoder<any>): _Element {
    if (!_cached_encoder_for_Ticket) { _cached_encoder_for_Ticket = $._encode_implicit(_TagClass.application, 1, () => function (value: Ticket, elGetter: $.ASN1Encoder<Ticket>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ $._encode_explicit(_TagClass.context, 0, () => $._encodeInteger, $.BER)(value.tkt_vno, $.BER),
            /* REQUIRED   */ $._encode_explicit(_TagClass.context, 1, () => _encode_Realm, $.BER)(value.realm, $.BER),
            /* REQUIRED   */ $._encode_explicit(_TagClass.context, 2, () => _encode_PrincipalName, $.BER)(value.sname, $.BER),
