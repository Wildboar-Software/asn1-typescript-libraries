...
class Checksum {
     ...
     * @returns {Checksum}
     */
    public static _from_object (_o: { [_K in keyof (Checksum)]: (Checksum)[_K] }): Checksum {
 ...
 */
export
const _root_component_type_list_1_spec_for_Checksum: $.ComponentSpec[] = [
    new $.ComponentSpec("cksumtype", false, $.hasTag(_TagClass.context, 0)),
    new $.ComponentSpec("checksum", false, $.hasTag(_TagClass.context, 1))
];
 ...
 */
export
const _root_component_type_list_2_spec_for_Checksum: $.ComponentSpec[] = [
 ...
 */
export
const _extension_additions_list_spec_for_Checksum: $.ComponentSpec[] = [
 ...
 */
export
function _decode_Checksum (el: _Element): Checksum {
    if (!_cached_decoder_for_Checksum) { _cached_decoder_for_Checksum = function (el: _Element): Checksum {
    const sequence: _Element[] = el.sequence;
    if (sequence.length < 2) {
        throw new _ConstructionError("Checksum contained only " + sequence.length.toString() + " elements.");
    }
    ...
    checksum = $._decode_explicit<OCTET_STRING>(() => $._decodeOctetString)(sequence[1]);
    return new Checksum(
        cksumtype,
        checksum,
 ...
 */
export
function _encode_Checksum (value: Checksum, elGetter: $.ASN1Encoder<any>): _Element {
    if (!_cached_encoder_for_Checksum) { _cached_encoder_for_Checksum = function (value: Checksum, elGetter: $.ASN1Encoder<Checksum>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ $._encode_explicit(_TagClass.context, 0, () => _encode_Int32, $.BER)(value.cksumtype, $.BER),
            /* REQUIRED   */ $._encode_explicit(_TagClass.context, 1, () => $._encodeOctetString, $.BER)(value.checksum, $.BER)
        ],
