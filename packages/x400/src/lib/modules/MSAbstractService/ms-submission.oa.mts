 ...
 * @description
 *
 * Information object `ms_submission`. The MS-submission Port abstract-services are
 * realized on a one-to-one basis between abstract-operations and real operations in the
 * Message Submission Service Element (MSSE) and MS Message Submission Service Element
 * (MS- MSSE) which are defined in Context. 17.3 Administration Port The Administration
 * Port abstract-services are realized on a one-to-one basis between abstract-operations
 * and real operations in the… See ITU-T X.413 (1999), §17.2.
 *
 * ### ASN.1 Definition:
 *
 * ```asn1
