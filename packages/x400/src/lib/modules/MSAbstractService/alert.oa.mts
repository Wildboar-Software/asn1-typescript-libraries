 ...
 * @description
 *
 * Information object `alert`. The Auto-alert auto-action enables the MS-user to instruct
 * the MS to alert the user automatically when the delivery of selected messages or
 * reports causes the creation of an entry in the Delivery entry-class. The alert may be
 * conveyed to the MS-user by means of the Alert abstract-operation (see 8.2.6). See
