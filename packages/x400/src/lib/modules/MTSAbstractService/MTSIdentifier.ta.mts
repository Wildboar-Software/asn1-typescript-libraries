 ...
 * @description
 *
 * `MTSIdentifier`. MTS-identifiers are assigned by the MTS to distinguish between
 * messages and probes at the MTS Abstract Service, and between messages, probes and
 * reports within the MTS. The MTS-identifier assigned to a message at a submission-port
 * ( message-submission-identifier) is identical to the corresponding message-identifier
 * at a transfer-port and corresponding message-delivery-identifier at a delivery-port.
 * See ITU-T X.411 (1999), §8.5.1.
 *
 * ### ASN.1 Definition:
 *
 * ```asn1
