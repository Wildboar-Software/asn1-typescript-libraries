 ...
 */
export
function _decode_Timer (el: _Element) {
    if (!_cached_decoder_for_Timer) { _cached_decoder_for_Timer = function (el: _Element): Timer {
    const sequence: _Element[] = el.sequence;
    if (sequence.length < 2) {
        throw new _ConstructionError("Timer contained only " + sequence.length.toString() + " elements.");
    }
    sequence[0].name = "exponent";
    sequence[1].name = "mantissa";
    let exponent!: INTEGER;
    let mantissa!: INTEGER;
    exponent = $._decode_implicit<INTEGER>(() => $._decodeInteger)(sequence[0]);
    mantissa = $._decode_implicit<INTEGER>(() => $._decodeInteger)(sequence[1]);
    return new Timer(
        exponent,
        mantissa,
 ...
 */
export
function _encode_Timer (value: Timer, elGetter: $.ASN1Encoder<Timer>) {
    if (!_cached_encoder_for_Timer) { _cached_encoder_for_Timer = function (value: Timer, elGetter: $.ASN1Encoder<Timer>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ $._encode_implicit(_TagClass.context, 1, () => $._encodeInteger, $.BER)(value.exponent, $.BER),
            /* REQUIRED   */ $._encode_implicit(_TagClass.context, 2, () => $._encodeInteger, $.BER)(value.mantissa, $.BER)
        ],
