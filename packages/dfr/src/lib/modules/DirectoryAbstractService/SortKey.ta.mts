...
class SortKey {
     ...
     * @returns {SortKey}
     */
    public static _from_object (_o: { [_K in keyof (SortKey)]: (SortKey)[_K] }): SortKey {
        return new SortKey(_o.type_, _o.orderingRule, _o._unrecognizedExtensionsList);
    }
 ...
 */
export
const _root_component_type_list_1_spec_for_SortKey: $.ComponentSpec[] = [
    new $.ComponentSpec("type", false, $.hasTag(_TagClass.universal, 6)),
    new $.ComponentSpec("orderingRule", true, $.hasTag(_TagClass.universal, 6))
];
 ...
 */
export
const _root_component_type_list_2_spec_for_SortKey: $.ComponentSpec[] = [
 ...
 */
export
const _extension_additions_list_spec_for_SortKey: $.ComponentSpec[] = [
...
function _decode_SortKey (el: _Element): SortKey {
    if (!_cached_decoder_for_SortKey) { _cached_decoder_for_SortKey = function (el: _Element): SortKey {
    let type_!: AttributeType;
    let orderingRule: OPTIONAL<OBJECT_IDENTIFIER>;
    let _unrecognizedExtensionsList: _Element[] = [];
    const callbacks: $.DecodingMap = {
        "type": (_el: _Element): void => { type_ = _decode_AttributeType(_el); },
        "orderingRule": (_el: _Element): void => { orderingRule = $._decodeObjectIdentifier(_el); }
    };
...
function _encode_SortKey (value: SortKey, elGetter: $.ASN1Encoder<any>): _Element {
    if (!_cached_encoder_for_SortKey) { _cached_encoder_for_SortKey = function (value: SortKey, elGetter: $.ASN1Encoder<SortKey>): _Element {
    return $._encodeSequence(([] as (_Element | undefined)[]).concat(
        [
            /* REQUIRED   */ _encode_AttributeType(value.type_, $.BER),
            /* IF_ABSENT  */ ((value.orderingRule === undefined) ? undefined : $._encodeObjectIdentifier(value.orderingRule, $.BER))
        ],
