 ...
 *
 * TypeScript encodings of ISO/IEC 10166 Document Filing and Retrieval
 * (DFR) ASN.1 modules and ISO/IEC 10031-2 Distributed Office
 * Referenced Data Transfer (DOR). Import from `@wildboar/dfr`.
 * Per-module subpaths remain available.
 *
 * Directory types (`Attribute`, `Filter`, `DistinguishedName`) are
 * re-exported from `@wildboar/x500`. Remote Operations types
 * (`OPERATION`, `ERROR`, `Code`, `InvokeId`) are re-exported from
 * `@wildboar/rose`. `PresentationAddress` is imported from
 * `@wildboar/x500/SelectedAttributeTypes`.
 *
 * `Priority` is the DFR ENUMERATED (`low` / `medium` / `high`), not
 * the ROSE INTEGER. `ID`, `id_mod`, and `id_mod_object_identifiers`
 * come from `DFRObjectIdentifiers`; the protocol-module copies are on
 * `@wildboar/dfr/DFRProtocolObjectIdentifiers`. The AccessRights short
 * name `read` is omitted because it collides with the `read` OPERATION;
 * use `AccessRights_read`.
 */
