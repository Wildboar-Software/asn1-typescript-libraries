 ...
 * @function
 */
export function defaultValueEncoder(value) {
    return "#" + Array.from(value.toBytes())
        .map((byte) => byte.toString(16).padStart(2, "0"))
        .join("");
}
export function distinguishedValueToString(value) {
...
export function attributeTypeAndValueToString(atav) {
    const key = distinguishedTypeToString(atav.type_);
    const value = distinguishedValueToString(atav.value);
    return `${key}=${value}`;
}
